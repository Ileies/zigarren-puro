...
<div class="flex min-h-screen items-center justify-center bg-base-200 px-4 py-16">
	<div class="w-full max-w-md rounded-2xl bg-base-100 p-8 shadow-sm ring-1 ring-base-300 text-center">
		{#if data.status === 'success'}
			...
			<p class="mb-6 text-sm text-base-content/60">
				Ihre E-Mail-Adresse wurde erfolgreich auf <strong class="text-base-content">{data.newEmail}</strong> geändert.
			</p>
			<a href="/account" class="btn btn-primary w-full">Zurück zum Konto</a>
		...
		{:else if data.status === 'email_taken'}
			...
			<p class="mb-6 text-sm text-base-content/60">
				Die gewünschte E-Mail-Adresse wird inzwischen von einem anderen Konto verwendet.
				Bitte fordern Sie eine neue Änderung mit einer anderen Adresse an.
			</p>
			<a href="/account" class="btn btn-primary w-full">Zurück zum Konto</a>
